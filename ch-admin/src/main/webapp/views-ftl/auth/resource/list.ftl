...
<#assign securityUtil = static["com.ch.base.util.SecurityUtil"]>
<!DOCTYPE html>
<html>
<head>
<title></title>
...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.ch.modalDialog({
			title : '添加资源信息',
			url : ch.contextPath + '/page/auth/resource/form',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var showFun = function(id) {
		var dialog = parent.ch.modalDialog({
			title : '查看资源信息',
			url : ch.contextPath + '/page/auth/resource/form?id=' + id
		});
	};
	...
	var removeFun = function(id) {
		parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
				$.post(ch.contextPath + '/auth/resource/delete', {
					id : id
				}, function() {
	...
	$(function() {
		grid = $('#grid').treegrid({
			title : '',
			url : ch.contextPath + '/auth/resource/treeGrid',
			idField : 'id',
			treeField : 'name',
			parentField : 'pid',
			rownumbers : true,
			...
			columns : [ [ {
				...
				formatter : function(value, row) {
					var str = '';
					<#if securityUtil.havePermission(loginUser,"/auth/resource/get")>
						str += ch.formatString('<img class="iconImg ext-icon-note" title="查看" onclick="showFun(\'{0}\');"/>', row.id);
					</#if>
					<#if securityUtil.havePermission(loginUser,"/auth/resource/update")>
						str += ch.formatString('<img class="iconImg ext-icon-note_edit" title="编辑" onclick="editFun(\'{0}\');"/>', row.id);
					</#if>
					<#if securityUtil.havePermission(loginUser,"/auth/resource/delete")>
...
<body class="easyui-layout" data-options="fit:true,border:false">
	<div id="toolbar" style="display: none;">
		<table>
			<tr>
				<#if securityUtil.havePermission(loginUser,"/auth/resource/save")>
