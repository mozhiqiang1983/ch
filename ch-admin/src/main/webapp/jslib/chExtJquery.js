 ...
 * 增加命名空间功能
 * 
 * 使用方法：ch.ns('jQuery.bbb.ccc','jQuery.eee.fff');
 */
ch.ns = function() {
	var o = {}, d;
 ...
 * 将form表单元素的值序列化成对象
 * 
 * @example ch.serializeObject($('#formId'))
 * 
 * 
 * 
 * @requires jQuery
 ...
 * @returns object
 */
ch.serializeObject = function(form) {
	var o = {};
 ...
 * 
 * 
 * @example ch.formatString('字符串{0}字符串{1}字符串','第一个变量','第二个变量');
 * 
 * @returns 格式化后的字符串
 */
ch.formatString = function(str) {
 ...
 * @returns list
 */
ch.stringToList = function(value) {
 ...
 * 
 * @example ch.cookie('the_cookie', 'the_value');
 * @desc Set the value of a cookie.
 * @example ch.cookie('the_cookie', 'the_value', { expires: 7, path: '/', domain: 'jquery.com', secure: true });
 * @desc Create a cookie with all available options.
 * @example ch.cookie('the_cookie', 'the_value');
 * @desc Create a session cookie.
 * @example ch.cookie('the_cookie', null);
 ...
 * @author Klaus Hartl/klaus.hartl@stilbuero.de
 */
ch.cookie = function(key, value, options) {
